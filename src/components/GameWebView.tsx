 ...
 * Images are pre-baked as base64 in gameAssets.ts (generated by
 * scripts/generate-assets.js) — fully self-contained, works in
 * Expo Go AND production APK builds with no runtime file reading.
 */
...
//  Assets come from GAME_ASSETS (pre-baked base64)
// ─────────────────────────────────────────────
function buildGameHTML(): string {
  const assetEntries = Object.entries(GAME_ASSETS)
...
export default function GameWebView() {
  ...
  // Build HTML once — GAME_ASSETS is already baked in at bundle time
  const html = useMemo(() => buildGameHTML(), []);
